...
- **Every executing method has an `_on` twin that runs on a connection the
  ...
  `execute_scoped_on` and their `_as` forms, and `ScopedEngine::query_on` /
  `run_on` and their `_as` forms (a scoped handle runs no compiled
  statements, as before), take any `sqlx::PgExecutor` — `&mut *tx` from a
  transaction the caller began, or the pool. The pool methods are now these
  twins bound to the engine's own pool, so each kind of statement has one
  execution path. A
  ...
  `ScopedTxClient` keep their API and now run through the same paths.

### Changed

- **Tracing.** The `executing` / `executing compiled` debug events carry
  `executor` (the executor's type: a pool or a borrowed connection — what
  tells a pool run from one inside a transaction) and `scoped`; the
  `in_tx` field from 0.21.0 and the "… in tx" / "… scoped" message variants
  are gone, folded into those two fields. Spans are opened by the `_on`
  methods and the transaction clients only, one per statement, instead of
  one per wrapper on the way down.

### Deprecated

- `executor::execute(pool, …)`: `execute_on` takes a pool as it takes any
  executor, and nothing in the crate calls the pool-only form any more.
